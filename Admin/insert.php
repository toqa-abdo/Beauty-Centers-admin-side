<?php
        ...
        if(isset($_POST['sub_btn']))
        {
            ...
            if( $user_name== null || $user_email== null ||$user_pass== null || $user_photo== null ){
                echo "<script>alert('one of information not inserted')</script>"; 
            }
            else{
            $result = $data->query("SELECT * FROM `centers` WHERE `center_name`  = '$user_name'");
            if($result->num_rows == 0) {
                 // row not found, do stuff...
                 $insert_data= "INSERT INTO `centers`( `center_name`, `center_email`, `center_password`, `center_image`) 
                 VALUES ('$user_name', '$user_email', '$user_pass', '$user_photo_folder' )";
